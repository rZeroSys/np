...
def calculate_valuation_metrics(row):
    ...
    elec_cost = safe_float(row.get('total_annual_electricity_cost'))
    gas_cost = safe_float(row.get('annual_gas_cost'))
    steam_cost = safe_float(row.get('annual_steam_cost'))
    fuel_oil_cost = safe_float(row.get('annual_fuel_oil_cost'))

    # Get HVAC percentages
    pct_elec_hvac = safe_float(row.get('pct_elec_hvac'))
    pct_gas_hvac = safe_float(row.get('pct_gas_hvac'))
    pct_steam_hvac = safe_float(row.get('pct_steam_hvac'))
    pct_fuel_oil_hvac = safe_float(row.get('pct_fuel_oil_hvac'))
    ...
    odcv_pct = safe_float(row.get('odcv_savings_pct'))
    ...
    fine_avoidance = safe_float(row.get('fine_avoidance_yr1'))
...
def process_file(input_file):
    ...
    # Add new columns
    new_cols = ['current_valuation_usd', 'post_odcv_valuation_usd', 'odcv_valuation_impact_usd',
                'total_annual_opex_avoidance', 'odcv_dollar_savings']
    for col in new_cols:
    ...
    for row in rows:
        ...
        row['current_valuation_usd'] = current_val
        row['post_odcv_valuation_usd'] = post_val
        row['odcv_valuation_impact_usd'] = impact
        row['total_annual_opex_avoidance'] = opex_avoidance
        row['odcv_dollar_savings'] = dollar_savings

        if row.get('building_type') in COMMERCIAL_TYPES:
            commercial_count += 1
    ...
    for row in rows:
        ...
        if bt in COMMERCIAL_TYPES:
            ...
            type_impacts[bt]['total_impact'] += safe_float(row.get('odcv_valuation_impact_usd'))
