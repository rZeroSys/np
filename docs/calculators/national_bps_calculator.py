...
def calc_fine_nyc(row):
    ...
    sqft = safe_float(row.get('square_footage'), 0)
    elec = safe_float(row.get('electricity_use_kbtu'), 0)
    gas = safe_float(row.get('natural_gas_use_kbtu'), 0)
    steam = safe_float(row.get('district_steam_use_kbtu'), 0)
    odcv_pct = safe_float(row.get('odcv_savings_pct'), 0)

    # HVAC percentages - use default only if energy exists but pct is NaN
    elec_hvac = safe_float(row.get('pct_elec_hvac'), DEFAULT_ELEC_HVAC) if elec > 0 else 0
    gas_hvac = safe_float(row.get('pct_gas_hvac'), DEFAULT_GAS_HVAC) if gas > 0 else 0
    steam_hvac = safe_float(row.get('pct_steam_hvac'), DEFAULT_STEAM_HVAC) if steam > 0 else 0
    ...
    fine_avoidance = (baseline_overage - with_odcv_overage) * params['fine_rate']

    return carbon_reduction, fine_avoidance
...
def calc_fine_denver(row):
    ...
    fine_avoidance = (baseline_overage_kbtu - with_odcv_overage_kbtu) * params['fine_rate']
...
def calc_fine_seattle(row):
    ...
    # If ODCV brings building into compliance
    if baseline_over_cap and not with_odcv_over_cap:
        fine_avoidance = sqft * params['fine_rate'] / params['cycle_years']
    else:
        fine_avoidance = 0
...
def calc_fine_sf(row):
    ...
    return carbon_reduction, 0  # No fine avoidance - no caps
...
def calc_fine_stlouis(row):
    ...
    # If ODCV brings building into compliance
    if not baseline_compliant and with_odcv_compliant:
        fine_avoidance = params['fine_rate'] * 365  # $500/day * 365
    else:
        fine_avoidance = 0
...
def process_csv(input_csv):
    ...
    df['city'] = df.apply(lambda row: extract_city(
        str(row.get('address', '')),
        str(row.get('state', ''))
    ), axis=1)

    city_fill = df['city'].notna().sum()
    print(f"Cities extracted: {city_fill}/{len(df)} ({100*city_fill/len(df):.2f}%)")

    # Initialize fine avoidance column only (carbon is calculated separately)
    df['fine_avoidance_yr1'] = 0.0

    # Process all buildings - exemptions handled per-city below
    all_buildings_mask = df['building_id'].notna()
    ...
    for city_name, calc_func in FINE_CALCULATORS.items():
        ...
        city_base_mask = df['city'] == city_name

        # Apply K-12/Government exemptions only for NYC and Denver
        if city_name in EXEMPT_K12_GOV:
            city_mask = city_base_mask & (df['vertical'] != 'Government') & (df['building_type'] != 'K-12')
        else:
        ...
        if city_count > 0:
            ...
            # Only update fine_avoidance - carbon is already calculated
            df.loc[city_mask, 'fine_avoidance_yr1'] = results[1]

            total_fine_avoidance = df.loc[city_mask, 'fine_avoidance_yr1'].sum()
            buildings_with_fines = (df.loc[city_mask, 'fine_avoidance_yr1'] > 0).sum()
    ...
    non_bps_count = (~df['city'].isin(FINE_CALCULATORS.keys())).sum()
    ...
    total_fines = df['fine_avoidance_yr1'].sum()
