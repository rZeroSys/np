...
def generate_hero(row):
    ...
    bldg_type = safe_val(row, 'bldg_type', 'Unknown')
...
"""
    ...
    if has_property_name:
        html += f'        <div class="loc_address">{escape(address)}</div>\n'
...
"""
    ...
    # Build tenant sub-org HTML with logo (if exists) - uses display name
    tenant_sub_html = ""
    if tenant_sub:
        tenant_sub_display = get_org_display_name(tenant_sub)
        tenant_sub_html = f"<div style='font-size:0.85em;color:#666;margin-top:3px;'>({escape(tenant_sub_display)})</div>{build_logo(tenant_sub)}"
    ...
    # Site EUI
    eui = safe_num(row, 'energy_site_eui')
    if eui:
        html += f"<tr><td>Site EUI{tooltip('energy_site_eui')}</td><td>{format_number(eui, 1)} kBtu/sqft</td></tr>"
...
"""
    ...
    gas_rate = safe_num(row, 'cost_gas_rate_therm')
...
"""
    ...
    steam_rate = safe_num(row, 'cost_steam_rate_mlb')
